 ...
 */
//
interface CompleteExerciseCardProps {
...
export default function CompleteExerciseCard({
  ...
  return (
    <>
      <div
        className={`mt-4 space-y-3 border-t pt-4 ${exercise.completed ? "opacity-60" : ""
          }`}
      >
        {/* Résumé des séries */}
        <div>
          <p className="font-semibold mb-2">Séries réalisées :</p>
          {exercise.actualSets?.map((set: ActualSet, setIndex: number) => (
